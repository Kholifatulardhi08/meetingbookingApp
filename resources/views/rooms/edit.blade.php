            ...
            <div class="col-md-8">
                    ...
                    <div class="card-body">
                        <form method="POST" action="{{ route('rooms.update', $rooms->id) }}">
                            ...
                            <div class="form-group row">
                                <label for="room_code"
                                    class="col-md-4 col-form-label text-md-right">{{ __('Room Code') }}</label>

                                <div class="col-md-6">
                                    <input id="room_code" type="text"
                                        class="form-control @error('room_code') is-invalid @enderror" name="room_code"
                                        value="{{ old('room_code', $rooms->room_code) }}" required>
                ...
                <div class="m-2 p-2">
                    <form method="POST" action="{{ route('rooms.destroy', $rooms->id) }}">
                        @csrf
                        @method('DELETE')
                        <button class="btn btn-danger">Delete {{ $rooms->romm_code }}</button>
                    </form>
                </div>
            </div>
        </div>
